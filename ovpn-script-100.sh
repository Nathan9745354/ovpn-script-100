...
echo "max-clients 100" >> /etc/openvpn/server/server.conf
sed -i 's/server 10.8.0.0 255.255.255.0/server 10.8.0.0 255.255.255.128/' /etc/openvpn/server/server.conf
...
iptables -A FORWARD -s 10.8.0.0/25 -j ACCEPT
...
./wondershaper -a tun0 -u 20000 -d 20000
